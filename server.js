  ...
  try {
    ...
    await client.query(`
      ...
      CREATE TABLE IF NOT EXISTS associates (
        badge TEXT PRIMARY KEY, login TEXT UNIQUE, name TEXT, home_dept TEXT, manager TEXT, shift_code TEXT,
        operation_mode TEXT, default_dept TEXT DEFAULT 'INBOUND',
        current_path_id INTEGER, current_station_id INTEGER, current_path_start BIGINT,
    ...
    `);
    // Add created_at column if it doesn't already exist (for existing databases)
    await client.query("ALTER TABLE associates ADD COLUMN IF NOT EXISTS created_at TIMESTAMP DEFAULT NOW()");
    await client.query(`
    ...
    if (parseInt(adminCount.rows[0].count) === 0) {
      await client.query(`INSERT INTO admin_profiles (name, login, pin, role) VALUES ($1, $2, $3, $4)`, ["System Admin", "admin", "1234", "Admin"]);
...
async function smartAssign(badge, targetDept, pool) {
  ...
  try {
    ...
    const pathsRes = await pool.query(
      `SELECT id, name, role_type, rotation_hours FROM paths WHERE mode = $1 AND active = true ORDER BY priority DESC`,
      [targetDept]
    );
    ...
    if (scored.length === 0) {
      return { error: "No eligible paths (check permissions or rotation cap)" };
    }
...
app.get("/api/floor-custom-renames", async (req, res) => {
  ...
  try { res.json({ renames: JSON.parse(row.rows[0].value) }); }
  catch { res.json({ renames: {} }); }
});
...
app.get("/api/floor-custom", async (req, res) => {
  ...
  try { res.json(JSON.parse(row.rows[0].value)); }
  catch { res.json({ paths: [], lines: [], stations: [] }); }
});
...
// ========== NEW ROUTES (Performance, Alerts, Leaderboard) ==========
app.post("/api/performance/upload", async (req, res) => {
  const csvData = req.body.csv;
  if (!csvData) return res.status(400).json({ error: "CSV data missing" });
  const rows = parseCSV(csvData);
  let inserted = 0;
  for (const r of rows) {
    const login = r.Login;
    const rate = parseFloat(r.Rate);
    const stationName = r.Station;
    if (!login || isNaN(rate) || !stationName) continue;
    const assoc = await pool.query("SELECT badge FROM associates WHERE login = $1", [login]);
    if (!assoc.rows.length) continue;
    const badge = assoc.rows[0].badge;
    const station = await pool.query(
      "SELECT s.id, l.name as line_name FROM stations s JOIN lines l ON s.line_id = l.id WHERE s.name = $1",
      [stationName]
    );
    if (!station.rows.length) continue;
    await pool.query(
      "INSERT INTO performance_rates (badge, login, station_id, line_name, station_name, rate) VALUES ($1,$2,$3,$4,$5,$6)",
      [badge, login, station.rows[0].id, station.rows[0].line_name, stationName, rate]
    );
    inserted++;
  }
  res.json({ success: true, inserted });
});
app.post("/api/performance/targets", async (req, res) => {
  const csvData = req.body.csv;
  if (!csvData) return res.status(400).json({ error: "CSV data missing" });
  const rows = parseCSV(csvData);
  for (const r of rows) {
    const stationName = r.Station;
    const target = parseFloat(r.TargetRate);
    if (!stationName || isNaN(target)) continue;
    const station = await pool.query(
      "SELECT s.id, l.name as line_name FROM stations s JOIN lines l ON s.line_id = l.id WHERE s.name = $1",
      [stationName]
    );
    if (!station.rows.length) continue;
    await pool.query(
      `INSERT INTO plan_rates (station_id, station_name, line_name, target_rate)
       VALUES ($1,$2,$3,$4)
       ON CONFLICT (station_id) DO UPDATE SET target_rate = EXCLUDED.target_rate, line_name = EXCLUDED.line_name`,
      [station.rows[0].id, stationName, station.rows[0].line_name, target]
    );
  }
  res.json({ success: true });
});
app.get("/api/performance/latest", async (req, res) => {
  const result = await pool.query(`
    SELECT DISTINCT ON (pr.station_id)
      pr.station_id, pr.station_name, pr.line_name,
      pr.badge, pr.login, a.name,
      pr.rate, pr.recorded_at,
      COALESCE(pl.target_rate, 0) as target_rate
    FROM performance_rates pr
    JOIN associates a ON pr.badge = a.badge
    LEFT JOIN plan_rates pl ON pr.station_id = pl.station_id
    ORDER BY pr.station_id, pr.recorded_at DESC
  `);
  res.json(result.rows);
});

app.post("/api/alerts/upload", async (req, res) => {
  const csvData = req.body.csv;
  if (!csvData) return res.status(400).json({ error: "CSV data missing" });
  const rows = parseCSV(csvData);
  const thresholds = await pool.query("SELECT * FROM alert_config");
  const thresholdMap = {};
  thresholds.rows.forEach(t => thresholdMap[t.category] = t.threshold_percent);
  const defaultThreshold = 10;
  let inserted = 0;
  for (const r of rows) {
    const category = r.Category;
    const stationName = r.Station;
    const planRate = parseFloat(r.PlanRate);
    const actualRate = parseFloat(r.ActualRate);
    if (!category || !stationName || isNaN(planRate) || isNaN(actualRate)) continue;
    const threshold = thresholdMap[category] ?? defaultThreshold;
    const deviation = planRate - actualRate;
    const deviationPercent = planRate > 0 ? (deviation / planRate) * 100 : 0;
    if (deviationPercent >= threshold) {
      ...
      await pool.query(
        `INSERT INTO alert_log (category, station_id, station_name, line_name, plan_rate, actual_rate, deviation)
         VALUES ($1,$2,$3,$4,$5,$6,$7)`,
        [category, station.rows[0]?.id || null, stationName, station.rows[0]?.line_name || '', planRate, actualRate, deviationPercent]
      );
      inserted++;
    }
  }
...
app.get("/api/alerts", async (req, res) => {
  const { category, acknowledged } = req.query;
...
app.patch("/api/alerts/:id", async (req, res) => {
  ...
  await pool.query(
    "UPDATE alert_log SET acknowledged = COALESCE($1, acknowledged), resolved = COALESCE($2, resolved) WHERE id = $3",
    [acknowledged, resolved, req.params.id]
  );
  res.json({ success: true });
});
app.post("/api/alerts/config", async (req, res) => {
  const { category, threshold_percent } = req.body;
  if (!category || threshold_percent === undefined)
    return res.status(400).json({ error: "category and threshold_percent required" });
  await pool.query(
    `INSERT INTO alert_config (category, threshold_percent) VALUES ($1,$2)
     ON CONFLICT (category) DO UPDATE SET threshold_percent = EXCLUDED.threshold_percent`,
    [category, threshold_percent]
  );
  res.json({ success: true });
});
app.get("/api/alerts/config", async (req, res) => {
  const result = await pool.query("SELECT * FROM alert_config");
  res.json(result.rows);
});

app.post("/api/quality/upload", async (req, res) => {
  const csvData = req.body.csv;
  if (!csvData) return res.status(400).json({ error: "CSV data missing" });
  const rows = parseCSV(csvData);
  for (const r of rows) {
    ...
    if (!badge || isNaN(score)) continue;
    await pool.query(
      `INSERT INTO quality_scores (badge, score) VALUES ($1,$2)
       ON CONFLICT (badge) DO UPDATE SET score = EXCLUDED.score`,
      [badge, score]
    );
  }
  res.json({ success: true });
...
app.get("/api/leaderboard", async (req, res) => {
  ...
  const highSidePathId = (await pool.query("SELECT id FROM paths WHERE name = 'CRETS High Side'")).rows[0]?.id;
  if (!highSidePathId) return res.json([]);

  const result = await pool.query(`
    SELECT a.badge, a.name, a.login,
      COALESCE(SUM(rh.hours), 0) as total_hours,
      COALESCE(qs.score, 80) as quality_score,
      MAX(p.lc_level) as lc_level
    FROM associates a
    LEFT JOIN role_hours rh ON a.badge = rh.badge AND rh.path_id = $1 AND rh.week_start >= $2
    LEFT JOIN permissions p ON a.badge = p.badge AND p.path_id = $1
    LEFT JOIN quality_scores qs ON a.badge = qs.badge
    WHERE a.active = true AND p.lc_level >= 2
    GROUP BY a.badge, a.name, a.login, qs.score
    HAVING SUM(rh.hours) > 0
    ORDER BY total_hours DESC
    LIMIT 10
  `, [highSidePathId, startOfMonth]);  // Removed the tenure subquery to avoid missing column error; you may add tenure check later if needed
  res.json(result.rows);
});
...
async function startServer() {
  try {
    ...
    const shift = await getSetting("active_shift") || "FHD";
    app.listen(PORT, () => {
      console.log(`\n🏭 Control Tower (TEN1) with Performance, Alerts & Leaderboard — http://localhost:${PORT}`);
      console.log(`📍 Mode: ${mode} | Shift: ${shift}\n`);
    });
  } catch (err) {
